...
def centerDCD(cwd,name,cycle, replicas, discard_first_nframes):
    # cwd = "/groups/sbinlab/fancao/IDPs_multi"  # current working directory
    residues = pd.read_csv(f'{cwd}/residues_{cycle-1}.csv').set_index('one', drop=False)
    prot = pd.read_pickle(f'{cwd}/allproteins.pkl').loc[name]
    top = md.Topology()
    chain = top.add_chain()
    ...
    traj = md.load_dcd(f"{cwd}/{prot.path}/0.dcd",top=top)[discard_first_nframes:]
    for i in range(1,replicas):
        t = md.load_dcd(f"{cwd}/{prot.path}/{i}.dcd",top=top)[discard_first_nframes:]
        traj = md.join([traj,t])
    ...
    traj.save_dcd(f'{cwd}/{prot.path}/{name}.dcd')
    traj[0].save_pdb(f'{cwd}/{prot.path}/{name}.pdb')
    # for i in range(replicas):
        # os.remove(f'{cwd}/{prot.path}/{i}.dcd')

starttime = time.time()  # begin timer
centerDCD(args.cwd, args.name, args.cycle, args.replicas, args.discard_first_nframes)
