...
def run_pulchra(cwd, dataset, prot_path,pulchra_path,i,frame):
    """
    This function runs Pulchra on a single frame.
    """
    name = f'{cwd}/{dataset}/{prot_path}/{i}.pdb'
    frame.save(name)
    FNULL = open(os.devnull, 'w')
    subprocess.run([pulchra_path,name],stdout=FNULL,stderr=FNULL)    
    outname = f'{cwd}/{dataset}/{prot_path}/{i}.rebuilt.pdb'
...
def reconstruct_pulchra(cwd, dataset, pulchra_path,prot_name,num_cpus):
    ...
    """
    print("reconstruct_pulchra....", prot_name)
    ...
    for j in range(1, t.n_frames, num_cpus):
        ...
        xyz = np.append( xyz, np.vstack(ray.get([run_pulchra.remote(cwd, dataset, prot.path,pulchra_path,i,t[i]) for i in range(j,n)])))
    ...
    print(prot_name,'has',allatom1.n_frames,'frames', "done")
